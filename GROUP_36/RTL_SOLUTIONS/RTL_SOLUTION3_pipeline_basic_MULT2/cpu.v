...
wire [31:0] instruction_MEM_WB = mem_wb_dout[33-:32];  // 修改提取位置
wire        reg_write_MEM_WB   = mem_wb_dout[1];         // 修改提取位置
wire        mem_2_reg_MEM_WB   = mem_wb_dout[0];
...
// 总计宽度 = 64*4 +32 +2+8 = 256+32+10 = 298 位
// 为便于说明，此处将各信号依次打包
// ------------------------------
wire [297:0] id_ex_dout;
reg_arstn_en #(
    .DATA_W(298)       // 修改 DATA_W 由 297 -> 298
) ID_EX_pipe (
    .clk    (clk),
    .arst_n (arst_n),
    .en     (enable),
    .din    ({
              pc_IF_ID,                // [297:234] 64b
              regfile_rdata_1,         // [233:170] 64b
              regfile_rdata_2,         // [169:106] 64b
              immediate_extended,      // [105:42]  64b
              instruction_IF_ID,       // [41:10]   32b
              alu_op,                  // [9:8]     2b
              alu_src,                 // [7]       1b
              reg_write,               // [6]       1b
              mem_read,                // [5]       1b
...
// 总宽度 = 64 + 64 + 64 + 32 + 6 = 230 位
// ------------------------------
wire [229:0] ex_mem_dout;
reg_arstn_en #(
    .DATA_W(230)       // 修改 DATA_W 由 250 -> 230
) EX_MEM_pipe (
    .clk    (clk),
    .arst_n (arst_n),
    .en     (enable),
...
wire [63:0] pc_EX_MEM         = ex_mem_dout[229-:64];
wire [63:0] alu_out_EX_MEM      = ex_mem_dout[165-:64];
wire [63:0] regfile_rdata_2_EX_MEM = ex_mem_dout[101-:64];
wire [31:0] instruction_EX_MEM  = ex_mem_dout[37-:32];
wire        reg_write_EX_MEM    = ex_mem_dout[5];
wire        mem_read_EX_MEM     = ex_mem_dout[4];
wire        mem_write_EX_MEM    = ex_mem_dout[3];
wire        mem_2_reg_EX_MEM    = ex_mem_dout[2];
wire        branch_EX_MEM       = ex_mem_dout[1];
wire        jump_EX_MEM         = ex_mem_dout[0];
